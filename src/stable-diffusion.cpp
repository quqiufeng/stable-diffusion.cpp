    ...
    sd::Tensor<float> sample(const std::shared_ptr<DiffusionModelRunner>& work_diffusion_model,
        ...
        auto denoise = [&](const sd::Tensor<float>& x, float sigma, int step) -> sd::guidance::GuiderOutput {
            ...
            auto run_condition = [&](const SDCondition& condition,
                ...
                diffusion_params.freeu_enabled = this->freeu_enabled;
                diffusion_params.freeu_b1 = this->freeu_b1;
                diffusion_params.freeu_b2 = this->freeu_b2;
                diffusion_params.freeu_s1 = this->freeu_s1;
                diffusion_params.freeu_s2 = this->freeu_s2;
...
static std::optional<ImageGenerationEmbeds> prepare_image_generation_embeds(sd_ctx_t* sd_ctx,
    ...
    if (sd_img_gen_params->ipadapter_tokens != nullptr &&
        ...
        cond.c_crossattn = sd::ops::concat(cond.c_crossattn, ipa_tensor, 1);

        // Also inject into unconditional conditioning if present
        if (!uncond.c_crossattn.empty()) {
            uncond.c_crossattn = sd::ops::concat(uncond.c_crossattn, ipa_tensor, 1);
        }

        LOG_INFO("IPAdapter: injected %d tokens (weight=%.2f) into conditioning, shape=%s",
                 num_tokens, weight,
